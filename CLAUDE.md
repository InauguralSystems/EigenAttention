...
## This design was mostly written by older models — question it

**Most of this repo was designed by Claude sessions running models many
generations old**, and there are no external consumers to break. A decision
you find in the tree carries **no authority from seniority** — it is a
hypothesis an earlier session committed. The tell is writing, or thinking,
*"X must be true because the code does Y."* Ask whether Y is a **law** (the
math, an external contract) or a **decision**; if a decision, price the
alternative before designing around it. When a design looks forced, check
whether the alternative already exists somewhere for another purpose.
